...
import { RunHandoffCallItem, RunHandoffOutputItem, RunMessageOutputItem, RunReasoningItem, RunToolApprovalItem, RunToolCallItem, RunToolCallOutputItem, RunInjectedMessageItem, } from "./items.mjs";
...
export async function executeToolsAndSideEffects(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state) {
    ...
    for (const functionResult of functionResults) {
        if (functionResult.type === 'function_output' &&
            functionResult.output &&
            typeof functionResult.output === 'object' &&
            functionResult.output.type === 'message') {
            // Create and add the injected message item
            const injectedMessageItem = new RunInjectedMessageItem(functionResult.output, agent);
        ...
        return withFunctionSpan(async (span) => {
            ...
            try {
                ...
                if (result &&
                    ...
                    const stringResult = 'Message with rich content injected into conversation';
                    ...
                    return {
                        ...
                        runItem: new RunToolCallOutputItem(getToolCallOutputItem(toolRun.toolCall, stringResult), agent, result),
                    };
                }
                // Use string data for tracing and event emitter
